    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        {giorniDisponibili.length > 0 && (
          <button onClick={() => setShowAddForm(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-[#D4AF37] text-white text-sm font-semibold rounded-lg hover:bg-[#C5A028] transition-colors">
      ...
      {showAddForm && (
        <form onSubmit={handleAddDay} className="bg-white rounded-xl border border-gray-200 p-6 flex flex-col sm:flex-row sm:items-end gap-4">
          ...
          <button type="submit" disabled={saving} className="px-4 py-2 bg-[#D4AF37] text-white text-sm font-semibold rounded-lg">{saving ? "..." : "Aggiungi"}</button>
          ...
          <div key={orario.giorno} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-b border-gray-200">
              ...
              <div className="flex gap-1">
                <button onClick={() => startEdit(orario)} className="p-1.5 text-gray-400 hover:text-[#D4AF37] transition-colors"><Pencil className="w-4 h-4" /></button>
            ...
            {editGiorno === orario.giorno ? (
              <div className="p-4 space-y-3">
                ...
                <div className="flex gap-3">
                  <button onClick={addCelebrazione} className="text-sm text-[#D4AF37] font-medium hover:underline">+ Aggiungi riga</button>
                  <button onClick={handleSave} disabled={saving} className="px-3 py-1.5 bg-[#D4AF37] text-white text-sm font-semibold rounded-lg">{saving ? "..." : "Salva"}</button>
