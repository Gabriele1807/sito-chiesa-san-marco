    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-surface rounded-xl border border-border p-6 space-y-4">
          <h3 className="text-lg font-bold text-foreground">Nuovo file privato</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-foreground/70 uppercase mb-1">Nome / Titolo</label>
              <input type="text" value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} required placeholder="Nome del file" className="w-full px-3 py-2 rounded-lg border border-border text-sm focus:outline-none focus:border-gold" />
            </div>
            <div>
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground/70 uppercase mb-1">Descrizione (opzionale)</label>
        ...
        <table className="w-full text-sm min-w-[600px]">
          <thead>
            <tr className="bg-background border-b border-border">
              ...
              <th className="text-left px-4 py-3 font-semibold text-foreground/70">Data caricamento</th>
              <th className="text-right px-4 py-3 font-semibold text-foreground/70">Azioni</th>
            </tr>
          </thead>
          <tbody>
            {files.map((f, i) => (
              <tr key={f.id} className={i % 2 === 0 ? "bg-surface" : "bg-background/50"}>
                <td className="px-4 py-3">
                  <p className="font-medium text-foreground">{f.nome}</p>
                  {f.descrizione && <p className="text-xs text-foreground/60 mt-0.5">{f.descrizione}</p>}
                </td>
                <td className="px-4 py-3 text-foreground/70">{f.tipo}</td>
                <td className="px-4 py-3 text-foreground/70">{new Date(f.dataCaricamento).toLocaleDateString("it-IT")}</td>
                <td className="px-4 py-3 text-right">
                  <a href={f.url} target="_blank" rel="noopener noreferrer" className="p-1.5 text-foreground/40 hover:text-gold transition-colors inline-block"><Download className="w-4 h-4" /></a>
                  <button onClick={() => setDeleteTarget(f)} className="p-1.5 text-foreground/40 hover:text-red-500 transition-colors ml-1"><Trash2 className="w-4 h-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {files.length === 0 && <p className="text-center py-8 text-foreground/40 text-sm">Nessun file privato caricato</p>}
      </div>
